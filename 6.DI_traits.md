# Dependency Injection, Traits или сказ о том, как иметь зависимости и получать от этого удовольствие!
...
```
Пока все хорошо, теперь мы должны подключить данный трейт к нашему классу.
...
Теперь класс Rick умеет логировать данные используя сторонний класс. Но как нам это понять?
В случае если это наш код  мы знаем что модем вызвать нужные методы, но как про это может знать клиентский код?
...
Давайте представим что нам нужно устанавливать пользователю локаль, которая передается посредством HTTP протокола:
...
Поскольку понятие принцип и паттерн довольно размыты и не дают четкого определения, возникает некая путаница - DI принцип или паттерн?
Позиция автора данной лекции такова: DI все же больще принцип как, например, принципы SOLID, а вот реализация различных менеджеров объектов - паттерны использующие принцип DI
